...
# TODO: different appraoch: nachschauen was schon drin ist, nicht so mit "last_category" usw... damit es tiefer und generischer geht
...
def get_tag_structure():
    ...
    for col in df.columns[3:]:
        ...
        if match:
            if match.group(1):
                category_0 = match.group(1)
                category_1 = match.group(2)
                if category_1:
                    tag_structure.append({f'category_0': category_0, 'category_1': category_1})
                else:
                    tag_structure.append({f'category_0': category_0})
            elif match.group(3):
                category_2 = match.group(3)
                category_3 = match.group(4)
                previous_tags = [tag for tag in tag_structure if 'category_1' in tag and tag['category_1'] == category_2]
                for tag in previous_tags:
                    new_tag = tag.copy()
                    new_tag['category_2'] = category_2
                    tag_structure.append(new_tag)
                    new_tag = tag.copy()
                    new_tag['category_2'] = category_2
                    new_tag['category_3'] = category_3
                    tag_structure.append(new_tag)
...
# def get_tag_structure():
#     tag_structure = []
#     categorystring_1 = None # the first category e.g. "Input Modalities" in "Input Modalities > Text / Tags / Categories (+) (+)" or "Graph" in "..> Graph > Similarity Graph"
#     categorystring_2 = None # the second category
#     for col in df.columns[3:]:
#         pattern = re.compile(r'^(?:(?!\.\.)(.*?)(?: > (.*))?|^\.\.> (.*) > (.*))$')
#         match = pattern.match(col)
#         if match:
#             categorystring_1 = match.group(1)
#             categorystring_2 = match.group(2)
#
#             # this could be the first encounter, e.g. "Input Modalities" and "Text / Tags / Categories (+) (+)" from "Input Modalities > Text / Tags / Categories (+) (+)"
#             # it could also be a second encounter, e.g. "..> Text / Tags / Categories (+) (+) > Comments" where "Text / Tags / Categories (+) (+)" is already in the list
#             # I want to have a tag with ONLY Input Modalities. I want to have a tag with "Input Modalities" and "Text / Tags / Categories (+) (+)", I want to
#             # have a tag with "Input Modalities" and "Text / Tags / Categories (+) (+)" and "Comments" etc etc
#
#             # iterate over the found match.groups
#
#
#
#
#
#             # if category_1 and not any(tag['category'] == category_1 for tag in tag_structure):
#             #     tag_structure.append({"category": category_1, "subcategory": "none", "subsubcategory": "none"})
#
#
#
#
#             # tag_structure.append({"category": category_1, "subcategory": category_2, "subsubcategory": None, "original_col": col})
#     return tag_structure
